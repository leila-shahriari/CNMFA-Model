                       ...
                       A = NULL)
{
  ...
  repeat {
    ...
    for (i in 1:g)
    {
      ...
      z.om.Phi = matrix(0, p, q)
      z.Phi = matrix(0, q, q)
      D11 = D12 = matrix(0, p, p)
      Ga = solve(S[, , i]) %*% A[, , i]
      Phi = diag(q) - t(Ga) %*% A[, , i]
      ...
      for (j in 1:num.na.class)
      {
        ...
        IDoo = diag(p) - diag(D[, i]) %*% C.ij
        z.Phi = z.Phi + sum(Z[ind, i]) * Phi
        z.om.Phi = z.om.Phi + sum(Z[ind, i]) * (IDoo %*% A[, , i] %*% Phi)
        D11 = D11 + sum(Z[ind, i]) * (IDoo %*% diag(D[, i]))
        D12 = D12 + sum(Z[ind, i]) * IDoo %*% A[, , i] %*% Phi %*% t(IDoo %*% A[, , i])
      }
      ...
      mu[, i] = colSums(Z[, i] * (VI[, i] + (1 - VI[, i]) / gam[i]) * 
                          (Y.hat[, , i] - t(A[, , i] %*% u.j))) /
        sum(Z[, i] * (VI[, i] + (1 - VI[, i]) / gam[i]))
      A1[, , i] = t(Z[, i] * (VI[, i] + (1 - VI[, i]) / gam[i]) * 
                      t(cent)) %*% t(u.j) + z.om.Phi
      A2[, , i] = u.j %*% (Z[, i] * (VI[, i] + (1 - VI[, i]) / 
                                       gam[i]) * t(u.j)) + z.Phi
      A[, , i] = A1[, , i] %*% solve(A2[, , i])
      u.cent = t(Y.hat[, , i]) - mu[, i] - A[, , i] %*% u.j
      D[, i] = (1 / ni[i]) * diag(u.cent %*% (Z[, i] * 
                                                (VI[, i] + (1 - VI[, i]) /
                                                   gam[i]) * t(u.cent)) + 
                                    D11 + D12 - 2 * z.om.Phi %*% t(A[, , i]) + 
                                    A[, , i] %*% z.Phi %*% t(A[, , i]))
