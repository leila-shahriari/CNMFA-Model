                         ...
                         A = NULL)
{
  ...
  for (i in 1:g)
  {
    ...
    for (j in 1:num.na.class)
    {
      ...
      wden[ind, i] = w[i] * (alpha[i] * dmvnorm(Y.o, mu.o, OSO) + (1 - alpha[i]) *
                               dmvnorm(Y.o, mu.o, gam[i] * OSO))
    }
  }
  ...
  if (isTRUE(Print)) {
    ...
    cat(cli::rule(
      center = cli::col_green(
        "* Fitting Mixture of Contaminated normal factor analysis (AECM) *"
      ),
      line_col = "deeppink3",
      line = 2
    ),
  ...
  repeat {
    ...
    ni = colSums(Z)  
    w = ni / n
    A1 = array(NA, dim = c(p, q, g))
    A2 = array(NA, dim = c(q, q, g))
    ...
    {
      cent = t(Y.hat[, , i]) - mu[, i]
      Soo.cent = matrix(NA, p, n)
      Ga = solve(S[, , i]) %*% A[, , i]
      V.i = matrix(0, p, p)
      Phi.i = diag(q) - t(Ga) %*% A[, , i]
      for (j in 1:num.na.class)
      {
        O = O.list[[j]]
        ind = ind.list[[j]]
        ind.e = matrix(cent[, ind], nrow = p)
        OSO = O %*% S[, , i] %*% t(O)
        ODO = O %*% diag(D[, i]) %*% t(O)
        S.ij = t(O) %*% solve(OSO) %*% O
        C.ij = t(O) %*% solve(ODO) %*% O
        
        Y.pat = matrix(Y.hh[ind, ] , ncol = p)
        Y.o = Y.pat %*% t(O)
        mu.o = as.vector(O %*% mu[, i])
        
        Correct = dmvnorm(Y.o, mu.o, gam[i]*OSO, log = T) - dmvnorm(Y.o, mu.o, OSO, log = T)
        VI[ind,i] = (alpha[i]) / (alpha[i] + (1 - alpha[i]) * exp(Correct))
        Soo.cent[, ind] = S.ij %*% ind.e
        
        #Delta = (diag(p) - S[, , i] %*% S.ij) %*% S[, , i]
        #z.Delta = sum(Z[ind, i]) * Delta
        #V.ij = ind.e %*% (Z[ind, i] * 
        #                    (VI[ind, i] + (1 - VI[ind, i]) / gam[i])  * 
        #                    t(ind.e)) + z.Delta
        
        #V.i = V.i + V.ij / ni[i]
      }
      Y.hat[, , i] = t(mu[, i] + S[, , i] %*% Soo.cent)
      
      
      
      mu[, i] = colSums(Z[, i] * (VI[, i] + (1 - VI[, i]) / gam[i])  * 
                          Y.hat[, , i]) / sum(Z[, i] * 
                                                (VI[, i] + (1 - VI[, i]) / gam[i]))
      alpha[i] = max(0.5, sum(Z[, i] * VI[, i]) / ni[i])
      if (alpha[i] == 1) alpha[i] = 0.99
      SS = A[,,i] %*% t(A[,,i]) + diag(D[,i])
      u.cent = t(Y.hat[,,i]) - mu[,i]
      
      gam[i] = max(1.001, 
                   sum(Z[, i] * (1 - VI[, i]) * 
                         diag(t(u.cent) %*% solve(SS) %*% u.cent)) /
                     sum(p * Z[, i] * (1 - VI[, i])) )
      ...
      A1[, , i] = V.i %*% Ga
      A2[, , i] = t(Ga) %*% V.i %*% Ga + Phi.i
      A[, , i] = A1[, , i] %*% solve(A2[, , i])
      D[, i] = diag(V.i - A[, , i] %*% t(Ga) %*% V.i)
